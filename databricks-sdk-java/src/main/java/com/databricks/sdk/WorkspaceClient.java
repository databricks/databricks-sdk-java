...
import com.databricks.sdk.core.DatabricksConfig;import com.databricks.sdk.mixin.ClustersExt;
...
public class WorkspaceClient {
   ...
   * grant rules are supported. A grant rule specifies a role assigned to a set
   * of principals. A list of rules attached to a resource is called a rule set.
   * A workspace must belong to an account for these APIs to work.
   */
   ...
   * condition of its result, and notifies one or more users and/or notification
   ...
   * scripts to the `allowlist` in UC so that users can leverage these artifacts
   * on compute configured with shared access mode.
   */
   ...
   * data centrally across all of the workspaces in a Databricks account. Users
   * in different workspaces can share access to the same data, depending on
   ...
   * own clusters (by fixing and hiding some values), control cost by limiting
   * per cluster maximum cost (by setting limits on attributes whose values
   * contribute to hourly price).
   * 
   ...
   * See the instance type pricing page for a list of the supported instance
   * types and their corresponding DBUs.
   * 
   ...
   * automated job. Databricks makes a distinction between all-purpose clusters
   * and job clusters. You use all-purpose clusters to analyze data
   ...
   * first step to managing external data sources within Unity Catalog, with the
   * second step being creating a data object (catalog, schema, or table) using
   * the connection. Data objects derived from a connection can be written to or
   * read from similar to other Unity Catalog data objects based on cloud
   * storage. Users may create different types of connections with each
   * connection having a unique set of configuration options to support
   ...
   * However, it can be useful to use dashboard objects to look-up a collection
   * of related query IDs. The API can also be used to duplicate multiple
   * dashboards at once since you can get a dashboard definition with a GET
   * request and then POST it to create a new one. Dashboards can be scheduled
   ...
   * creating a query object, you may optionally specify a `data_source_id` for
   * the SQL warehouse against which it will run. If you don't already know the
   * `data_source_id` for your desired SQL warehouse, this API will help you
   * find it.
   * 
   ...
   * not have access to an external location in Unity Catalog, the request fails
   * and Unity Catalog does not attempt to authenticate to your cloud tenant on
   * the user’s behalf.
   * 
   ...
   * 
   * The function implementation can be any SQL expression or Query, and it can
   * be invoked wherever a table reference is allowed in a query. In Unity
   ...
   * init script returns with a bad exit code, the Apache Spark container fails
   * to launch and init scripts with later position are skipped. If enough
   * containers fail, the entire cluster fails with a
   ...
   * access to data in a metastore. Access can be granted by either a metastore
   * admin, the owner of an object, or the owner of the catalog or schema that
   * contains the object. Securable objects in Unity Catalog are hierarchical
   ...
   * auto-scaling times.
   * 
   * Databricks pools reduce cluster start and auto-scaling times by maintaining
   * a set of idle, ready-to-use instances. When a cluster is attached to a
   * pool, cluster nodes are created using the pool’s idle instances. If the
   * pool has no idle instances, the pool expands by allocating a new instance
   * from the instance provider in order to accommodate the cluster’s request.
   * When a cluster releases an instance, it returns to the pool and is free for
   * another cluster to use. Only clusters attached to a pool can use that
   ...
   * 
   * When a connection is attempted: 1. **First, all block lists are checked.**
   * If the connection IP address matches any block list, the connection is
   * rejected. 2. **If the connection was not rejected by block lists**, the IP
   * address is compared with the allow lists.
   * 
   ...
   * single task or can be a large, multi-task workflow with complex
   * dependencies. Databricks manages the task orchestration, cluster
   * management, monitoring, and error reporting for all of your jobs. You can
   * run your jobs immediately or periodically through an easy-to-use scheduling
   * system. You can implement job tasks using notebooks, JARS, Delta Live
   * Tables pipelines, or Python, Scala, Spark submit, and Java applications.
   * 
   ...
   * 
   * To make third-party or custom code available to notebooks and jobs running
   * on your clusters, you can install a library. Libraries can be written in
   * Python, Java, Scala, and R. You can upload Java, Scala, and Python
   * libraries and point to external packages in PyPI, Maven, and CRAN
   * repositories.
   * 
   ...
   * PyPI or Maven, using a previously installed workspace library, or using an
   * init script.
   * 
   ...
   * when you restart the cluster. Until you restart the cluster, the status of
   * the uninstalled library appears as Uninstall pending restart.
   */
   ...
   * them to Databricks workspaces to control which workloads use each
   ...
   * includes a legacy Hive metastore, the data in that metastore is available
   * in a catalog named hive_metastore.
   */
   ...
   * world who shares the data. A provider contains shares which further contain
   * the shared data.
   */
   ...
   * description, tags, parameters, and visualizations. Queries can be scheduled
   * using the `sql_task` type of the Jobs API, e.g. :method:jobs/create.
   */
   ...
   * recipient follows the activation link shared by the data provider to
   * download the credential file that includes the access token. The recipient
   * will then use the credential file to establish a secure connection with the
   * provider to receive the shared data.
   * 
   * Note that you can download the credential file only once. Recipients should
   * treat the downloaded credential as a secret and must not share it outside
   ...
   * the key identifier that enables the secure connection. This sharing mode is
   * called **Databricks-to-Databricks sharing**.
   * 
   * - For recipients without access to a Databricks workspace that is enabled
   * for Unity Catalog, when you create a recipient object, Databricks generates
   * an activation link you can send to the recipient. The recipient follows the
   * activation link to download the credential file, and then uses the
   * credential file to establish a secure connection to receive the shared
   ...
   * pulling, branch management, and visual comparison of diffs when committing.
   * 
   ...
   * access (or list) a table or view in a schema, users must have the
   ...
   * notebook, use Databricks secrets to store your credentials and reference
   * them in notebooks and jobs.
   * 
   ...
   * values that might be displayed in notebooks, it is not possible to prevent
   ...
   * apps, and CI/CD platforms. Databricks recommends creating service
   * principals to run production jobs or modify production data. If all
   * processes that act on production data run with service principals,
   * interactive users do not need any write, delete, or modify privileges in
   * production. This eliminates the risk of a user overwriting production data
   * by accident.
   */
   ...
   * and will not appear in your cloud account. A serving endpoint can consist
   * of one or more MLflow models from the Databricks Model Registry, called
   * served models. A serving endpoint can have at most ten served models. You
   * can configure traffic settings to define how requests should be routed to
   * your served models behind an endpoint. Additionally, you can configure the
   ...
   * defined within the metastore using :method:shares/update. You can register
   * data assets under their original name, qualified by their original schema,
   * or provide alternate exposed names.
   */
   ...
   * seconds (default: 10) the call behaves synchronously and waits for results
   * up to the specified timeout; when set to `0s`, the call is asynchronous and
   * responds immediately with a statement ID that can be used to poll for
   * status or fetch the results in a separate call.
   * 
   ...
   * The `result` field contains the first chunk of result data according to the
   * specified `disposition`, and links to fetch any remaining chunks.
   * 
   * If the execution does not complete before `wait_timeout`, the setting
   * `on_wait_timeout` determines how the system responds.
   * 
   * By default, `on_wait_timeout=CONTINUE`, and after reaching `wait_timeout`,
   * a response is returned and statement execution continues asynchronously.
   * The response will contain only `statement_id` and `status` fields, and the
   * caller must now follow the flow described for asynchronous call mode to
   * poll and fetch the result.
   * 
   * Alternatively, `on_wait_timeout` can also be set to `CANCEL`; in this case
   * if the timeout is reached before execution completes, the underlying
   * statement execution is canceled, and a `CANCELED` status is returned in the
   * response.
   * 
   ...
   * identical to those in the response to a successful synchronous submission.
   * The `result` field will contain the first chunk of result data, either
   * `INLINE` or as `EXTERNAL_LINKS` depending on `disposition`. Additional
   * chunks of result data can be fetched by checking for the presence of the
   * `next_chunk_internal_link` field, and iteratively `GET` those paths until
   * that field is unset: `GET
   ...
   * 
   * You can also configure how to fetch the result data in two different modes
   * by setting the `disposition` field to `INLINE` or `EXTERNAL_LINKS`.
   * 
   ...
   * **Presigned URLs**
   * 
   * External links point to data stored within your workspace's internal DBFS,
   * in the form of a presigned URL. The URLs are valid for only a short period,
   * <= 15 minutes. Alongside each `external_link` is an expiration field
   * indicating the time at which the URL is no longer valid. In
   ...
   * from cloud storage. As the short-lived credential is embedded in a
   * pre-signed URL, this URL should be protected.
   * 
   ...
   * approximate, occur server-side, and cannot account for caller delays,
   * network latency from caller to service, and similarly. - After a statement
   * has been submitted and a statement_id is returned, that statement's status
   * and result will automatically close after either of 2 conditions: - The
   * last result chunk is fetched (or resolved to an external link). - One hour
   * passes with no calls to get the status or fetch the result. Best practice:
   * in asynchronous clients, poll for status regularly (and with backoff) to
   ...
   * credential is subject to Unity Catalog access-control policies that control
   * which users and groups can access the credential. If a user does not have
   * access to a storage credential in Unity Catalog, the request fails and
   ...
   * schema may contain information about customer usage of Unity Catalog such
   * as audit-logs, billing-logs, lineage information, etc.
   */
   ...
   * key is the parent constraint of the foreign key and the table this primary
   * key is on is the parent table of the foreign key. Similarly, the foreign
   * key is the child constraint of its referenced primary key; the table of the
   * foreign key is the child table of the primary key.
   * 
   ...
   * USE_CATALOG permission on its parent catalog and the USE_SCHEMA permission
   * on its parent schema.
   * 
   * A table can be managed or external. From an API perspective, a __VIEW__ is
   * a particular kind of table (rather than a managed or external table).
   */
   ...
   * longer needs access to Databricks workspace, admins can terminate the user
   * in your identity provider and that user’s account will also be removed
   * from Databricks workspace. This ensures a consistent offboarding process
