...
public class ClustersExt extends ClustersAPI {
    ...
    public String selectSparkVersion(SparkVersionSelector selector) throws IllegalArgumentException {
        ...
        for (SparkVersion version : sv.getVersions()) {
            ...
            boolean matches = ((!version.getKey().contains("apache-spark-"))
    ...
    public String selectNodeType(NodeTypeSelector selector) {
        ...
        List<NodeType> types = res.getNodeTypes().stream()
                .sorted(nodeSortingComparator)
        ...
        for (NodeType nt : types) {
            ...
            if (selector.localDisk != null || selector.localDiskMinSize != null) {
                ...
                long localNvmeDisks = instanceType.getLocalNvmeDisks() != null ? instanceType.getLocalNvmeDisks() : 0;
                if (localDisks < 1 && localNvmeDisks < 1) {
                    continue;
                }
                Long localDiskSizeGb = instanceType.getLocalDiskSizeGb() != null ? instanceType.getLocalDiskSizeGb() : 0;
                Long localNvmeDiskSizeGb = instanceType.getLocalNvmeDiskSizeGb() != null ? instanceType.getLocalNvmeDiskSizeGb() : 0;
            ...
            if (selector.isIoCacheEnabled != null && !nt.getIsIoCacheEnabled().equals(selector.isIoCacheEnabled)) {
                continue;
            }
            if (selector.supportPortForwarding != null && !nt.getSupportPortForwarding().equals(selector.supportPortForwarding)) {
                continue;
            }
            if (selector.photonDriverCapable != null && !nt.getPhotonDriverCapable().equals(selector.photonDriverCapable)) {
                continue;
            }
            if (selector.photonWorkerCapable != null && !nt.getPhotonWorkerCapable().equals(selector.photonWorkerCapable)) {
                continue;
            }
    ...
    private final Comparator<NodeType> nodeSortingComparator = (item1, item2) ->
...
//            start(clusterId).get();
//        } else if (Arrays.asList(State.PENDING, State.RESIZING, State.RESTARTING).contains(info.getState())) {
//            waitGetClusterRunning(clusterId);
//        } else if (Arrays.asList(State.ERROR, State.UNKNOWN).contains(info.getState())) {
//            throw new RuntimeException("Cluster " + info.getClusterName() + " is " + info.getState() + ": " + info.getStateMessage());
//        }
//    }
}
