...
public class AccountClient {
   ...
   * exceeding a budget for a period. They can also retrieve the status of each
   * budget.
   */
   ...
   * needs access to a cross-account service IAM role in your AWS account so
   * that Databricks can deploy clusters in the appropriate VPC for the new
   * workspace. A credential configuration encapsulates this role information,
   * and its ID is used when creating a new workspace.
   */
   ...
   * and some information about how the key configuration can be used. There are
   * two possible uses for key configurations:
   * 
   ...
   * new workspace. This Preview feature is available if your account is on the
   * E2 version of the platform. Updating a running workspace with workspace
   * storage encryption requires that the workspace is on the E2 version of the
   * platform. If you have an older workspace, it might not be on the E2 version
   * of the platform. If you are not sure, contact your Databricks
   * representative.
   */
   ...
   * to the account console and account APIs. If the feature is disabled for the
   ...
   * provided to know the latest status of log delivery attempts. The high-level
   * flow of billable usage delivery:
   * 
   ...
   * AWS IAM role. For full details, including the required IAM role policies
   * and trust relationship, see [Billable usage log delivery]. Using Databricks
   ...
   * to all current and future workspaces plus account level logs, while
   * workspace level log delivery solely delivers logs related to the specified
   ...
   * `<bucket-name>/<prefix>/billable-usage/csv/`, where `<prefix>` is the name
   * of the optional delivery path prefix you set up during log delivery
   ...
   * workspaces in your account. * The files are delivered daily by overwriting
   * the month's CSV file for each workspace.
   * 
   ...
   * has a default value, this setting is present on all accounts even though
   * it's never set on a given account. Deletion reverts the value of the
   * setting back to the default value.
   */
  ...
  /**
   * These APIs enable administrators to enroll OAuth for their accounts, which
   * is required for adding/using any OAuth published/custom application
   * integration.
   * 
   ...
   * service principal, which can then be used to access Databricks Accounts and
   * Workspace APIs. For more information, see [Authentication using OAuth
   ...
   * encapsulates this bucket information, and its ID is used when creating a
   * new workspace.
   */
   ...
   * streamlines onboarding a new employee or team by using your identity
   * provider to create users and groups in Databricks account and give them the
   * proper level of access. When a user leaves your organization or no longer
   * needs access to Databricks account, admins can terminate the user in your
   * identity provider and that user’s account will also be removed from
   * Databricks account. This ensures a consistent offboarding process and
