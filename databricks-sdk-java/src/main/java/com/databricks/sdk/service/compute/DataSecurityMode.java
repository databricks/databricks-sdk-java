 ...
 * they cannot see each other's data and credentials. Most data governance features are supported in
 * this mode. But programming languages and cluster features might be limited. * `LEGACY_TABLE_ACL`:
 * This mode is for users migrating from legacy Table ACL clusters. * `LEGACY_PASSTHROUGH`: This
 * mode is for users migrating from legacy Passthrough on high concurrency clusters. *
