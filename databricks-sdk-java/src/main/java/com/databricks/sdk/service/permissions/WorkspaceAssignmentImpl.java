...
class WorkspaceAssignmentImpl implements WorkspaceAssignmentService {
    ...
    public void delete(DeleteWorkspaceAssignmentRequest request) {
        String path = String.format("/api/2.0/accounts//workspaces/%s/permissionassignments/principals/%s", request.getWorkspaceId(), request.getPrincipalId());
        apiClient.DELETE(path, request, Void.class);
    }
    
    @Override
    public WorkspacePermissions get(GetWorkspaceAssignmentRequest request) {
        String path = String.format("/api/2.0/accounts//workspaces/%s/permissionassignments/permissions", request.getWorkspaceId());
    ...
    public PermissionAssignments list(ListWorkspaceAssignmentRequest request) {
        String path = String.format("/api/2.0/accounts//workspaces/%s/permissionassignments", request.getWorkspaceId());
