...
public class AwsAttributes {
   ...
   * IAM instance profile. The instance profile must have previously been added
   * to the Databricks environment by an account administrator.
   * 
   ...
   * is half of the price of on-demand `r3.xlarge` instances. Similarly, if this
   ...
   * cluster creation, and if not specified, a default zone will be used. If the
   * zone specified is "auto", will try to place cluster in a zone with high
   * availability, and will retry placement in a different AZ if there is not
   * enough capacity. See [[AutoAZHelper.scala]] for more details. The list of
  ...
  @Override
  public int hashCode() {
    return Objects.hash(availability, ebsVolumeCount, ebsVolumeIops, ebsVolumeSize, ebsVolumeThroughput, ebsVolumeType, firstOnDemand, instanceProfileArn, spotBidPricePercent, zoneId);
  }
