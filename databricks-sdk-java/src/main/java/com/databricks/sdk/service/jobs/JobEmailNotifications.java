...
public class JobEmailNotifications {
  /**
   * If true, do not send email to recipients specified in {@code on_failure} if the run is skipped.
   * This field is {@code deprecated}. Please use the {@code
   ...
   * specified for the {@code RUN_DURATION_SECONDS} metric in the {@code health} field. If no rule
   * for the {@code RUN_DURATION_SECONDS} metric is specified in the {@code health} field for the
   * job, notifications are not sent.
   */
   ...
   * considered to have completed unsuccessfully if it ends with an {@code INTERNAL_ERROR} {@code
   * life_cycle_state} or a {@code FAILED}, or {@code TIMED_OUT} result_state. If this is not
   ...
   * stream. Streaming backlog thresholds can be set in the {@code health} field using the following
   * metrics: {@code STREAMING_BACKLOG_BYTES}, {@code STREAMING_BACKLOG_RECORDS}, {@code
   * STREAMING_BACKLOG_SECONDS}, or {@code STREAMING_BACKLOG_FILES}. Alerting is based on the
   ...
   * to have completed successfully if it ends with a {@code TERMINATED} {@code life_cycle_state}
   * and a {@code SUCCESS} result_state. If not specified on job creation, reset, or update, the
