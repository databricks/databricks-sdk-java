 ...
 * to service. - To keep a statement alive, poll for its status at least once every 15 minutes.
 * Regular polling guarantees the statement stays alive. - To stop a statement you no longer need,
 * cancel it explicitly with :method:statementexecution/cancelExecution. Stopping polling does not
 * cancel a statement. Explicit cancellation is the only way to end statement execution on demand. -
 * The results are only available for one hour after success; polling does not extend this. - The
 * SQL Execution API must be used for the entire lifecycle of the statement. For example, you cannot
...
public interface StatementExecutionService {
   ...
   *
   * <p>For flows that generate small and predictable result sets (<= 25 MiB), `INLINE` responses of
   * `JSON_ARRAY` result data are typically the simplest way to execute and fetch result data.
   *
   * <p>**Use case: large result sets with EXTERNAL_LINKS**
   *
   ...
   * <p>----
   *
   * <p>StatementResponse contains `statement_id` and `status`; other fields might be absent or
