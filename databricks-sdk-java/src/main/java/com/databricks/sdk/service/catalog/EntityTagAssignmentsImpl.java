...
class EntityTagAssignmentsImpl implements EntityTagAssignmentsService {
  ...
  public void delete(DeleteEntityTagAssignmentRequest request) {
    String path =
        String.format(
            "/api/2.1/unity-catalog/entity-tag-assignments/%s/tags/%s",
            request.getEntityName(), request.getTagKey());
    try {
  ...
  public ListEntityTagAssignmentsResponse list(ListEntityTagAssignmentsRequest request) {
    String path =
        String.format(
            "/api/2.1/unity-catalog/entity-tag-assignments/%s/tags", request.getEntityName());
    try {
      Request req = new Request("GET", path);
      ApiClient.setQuery(req, request);
      req.withHeader("Accept", "application/json");
