...
public class StatementExecutionAPI {
  ...
  public GetStatementResponse getStatement(String statementId) {
    return getStatement(new GetStatementRequest()
      .setStatementId(statementId));
  }
   ...
   * and the first chunk of the result data. When the statement is in the
   * terminal states `CANCELED`, `CLOSED` or `FAILED`, it returns HTTP 200 with
   * the state set. After at least 12 hours in terminal state, the statement is
  ...
  public ResultData getStatementResultChunkN(String statementId, long chunkIndex) {
    return getStatementResultChunkN(new GetStatementResultChunkNRequest()
      .setStatementId(statementId)
