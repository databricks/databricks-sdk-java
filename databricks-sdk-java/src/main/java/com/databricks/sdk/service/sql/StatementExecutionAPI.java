 ...
 * `format=ARROW_STREAM` and `disposition=EXTERNAL_LINKS` allows large result sets, and with higher
 * throughput.
 *
 ...
 * last result chunk is fetched (or resolved to an external link). - Ten (10) minutes pass with no
 * calls to get status or fetch result data. Best practice: in asynchronous clients, poll for status
 * regularly (and with backoff) to keep the statement open and alive. - After a `CANCEL` or `CLOSE`
 * operation, the statement will no longer be visible from the API which means that a subsequent
 * poll request may return an HTTP 404 NOT FOUND error. - After fetching the last result chunk
 * (including chunk_index=0), the statement is closed; shortly after closure the statement will no
 * longer be visible to the API and so, further calls such as
 * :method:statementexecution/getStatement may return an HTTP 404 NOT FOUND error.
 *
...
public class StatementExecutionAPI {
   ...
   *
   * <p>Polls for the statement's status; when `status.state=SUCCEEDED` it will also return the
   * result manifest and the first chunk of the result data.
   *
   ...
   *
   * <p>After statement execution has SUCCEEDED, result data can be fetched by chunks.
   *
   * <p>The first chunk (`chunk_index=0`) is typically fetched through `getStatementResult`, and
   * subsequent chunks with this call. The response structure is identical to the nested `result`
   * element described in getStatementResult, and similarly includes `next_chunk_index` and
