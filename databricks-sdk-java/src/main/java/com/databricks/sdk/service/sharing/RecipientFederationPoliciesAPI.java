 ...
 *
 * <ul>
 *   <li>User-to-Machine (U2M) flow: A user accesses Delta Shares using their own identity, such as
 *       connecting through PowerBI Delta Sharing Client.
 *   <li>Machine-to-Machine (M2M) flow: An application accesses Delta Shares using its own identity,
 *       typically for automation tasks like nightly jobs through Python Delta Sharing Client. OIDC
 *       Token Federation enables fine-grained access control, supports Multi-Factor Authentication
 *       (MFA), and enhances security by minimizing the risk of credential leakage through the use
 *       of short-lived, expiring tokens. It is designed for strong identity governance, secure
...
public class RecipientFederationPoliciesAPI {
   ...
   * using OIDC tokens.
   *
   * <p>Supported scenarios for federation policies:
   *
   * <ol>
   *   <li><b>User-to-Machine (U2M) flow</b> (e.g., PowerBI): A user accesses a resource using their
   *       own identity.
   *   <li><b>Machine-to-Machine (M2M) flow</b> (e.g., OAuth App): An OAuth App accesses a resource
   *       using its own identity, typically for tasks like running nightly jobs.
   * </ol>
   *
   * <p>For an overview, refer to:
   *
   * <ul>
   *   <li>Blog post: Overview of feature:
   *       https://www.databricks.com/blog/announcing-oidc-token-federation-enhanced-delta-sharing-security
   * </ul>
   *
   * <p>For detailed configuration guides based on your use case:
   *
   * <ul>
   *   <li>Creating a Federation Policy as a provider:
   *       https://docs.databricks.com/en/delta-sharing/create-recipient-oidc-fed
   *   <li>Configuration and usage for Machine-to-Machine (M2M) applications (e.g., Python Delta
   *       Sharing Client): https://docs.databricks.com/aws/en/delta-sharing/sharing-over-oidc-m2m
   *   <li>Configuration and usage for User-to-Machine (U2M) applications (e.g., PowerBI):
