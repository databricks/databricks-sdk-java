...
public interface TablesService {
   ...
   *
   * <p>To create an external delta table, the caller must have the <b>EXTERNAL_USE_SCHEMA</b>
   * privilege on the parent schema and the <b>EXTERNAL_USE_LOCATION</b> privilege on the external
   * location. These privileges must always be granted explicitly, and cannot be inherited through
   * ownership or <b>ALL_PRIVILEGES</b>.
   *
   * <p>Standard UC permissions needed to create tables still apply: <b>USE_CATALOG</b> on the
   * parent catalog (or ownership of the parent catalog), <b>CREATE_TABLE</b> and <b>USE_SCHEMA</b>
   * on the parent schema (or ownership of the parent schema), and <b>CREATE_EXTERNAL_TABLE</b> on
   * external location.
   *
   * <p>The <b>columns</b> field needs to be in a Spark compatible format, so we recommend you use
   ...
   * Runtime.
   *
   * <p>NOTE: The Create Table API for external clients only supports creating <b>external delta
   * tables</b>. The values shown in the respective enums are all values supported by Databricks,
   * however for this specific Create Table API, only <b>table_type</b> <b>EXTERNAL</b> and
   * <b>data_source_format</b> <b>DELTA</b> are supported. Additionally, column masks are not
   ...
   * the parent catalog, have the <b>USE_CATALOG</b> privilege on the parent catalog and be the
   * owner of the parent schema, or be the owner of the table and have the <b>USE_CATALOG</b>
   * privilege on the parent catalog and the <b>USE_SCHEMA</b> privilege on the parent schema.
   */
   ...
   *
   * <ul>
   *   <li>Be a metastore admin
   *   <li>Be the owner of the parent catalog
   *   <li>Be the owner of the parent schema and have the <b>USE_CATALOG</b> privilege on the parent
   *       catalog
   *   <li>Have the <b>USE_CATALOG</b> privilege on the parent catalog and the <b>USE_SCHEMA</b>
   *       privilege on the parent schema, and either be the table owner or have the <b>SELECT</b>
   *       privilege on the table.
   *   <li>Have <b>BROWSE</b> privilege on the parent catalog
   *   <li>Have <b>BROWSE</b> privilege on the parent schema
   * </ul>
   */
   ...
   * caller must be a metastore admin or an owner of (or have the <b>SELECT</b> privilege on) the
   * table. For the latter case, the caller must also be the owner or have the <b>USE_CATALOG</b>
   * privilege on the parent catalog and the <b>USE_SCHEMA</b> privilege on the parent schema. There
   * is no guarantee of a specific ordering of the elements in the array.
   *
   * <p>NOTE: <b>view_dependencies</b> and <b>table_constraints</b> are not returned by ListTables
   * queries.
   *
   ...
   *
   * <ul>
   *   <li>summaries for tables (within the current metastore and parent catalog and schema), when
   *       the user is a metastore admin, or:
   *   <li>summaries for tables and schemas (within the current metastore and parent catalog) for
   *       which the user has ownership or the <b>SELECT</b> privilege on the table and ownership or
   *       <b>USE_SCHEMA</b> privilege on the schema, provided that the user also has ownership or
   *       the <b>USE_CATALOG</b> privilege on the parent catalog.
   * </ul>
   *
   ...
   * <b>USE_CATALOG</b> privilege on the parent catalog and be the owner of the parent schema, or be
   * the owner of the table and have the <b>USE_CATALOG</b> privilege on the parent catalog and the
   * <b>USE_SCHEMA</b> privilege on the parent schema.
   */
