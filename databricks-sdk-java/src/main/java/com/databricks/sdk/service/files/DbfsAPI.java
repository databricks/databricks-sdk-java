...
public class DbfsAPI {
   ...
   * an error message (503 Service Unavailable) asking you to re-invoke the
   * delete operation until the directory structure is fully deleted.
   * 
  ...
  public FileInfo getStatus(String path) {
    return getStatus(new GetStatusRequest()
      .setPath(path));
  }
  ...
  public Iterable<FileInfo> list(String path) {
    return list(new ListDbfsRequest()
      .setPath(path));
  }
  ...
  public ReadResponse read(String path) {
    return read(new ReadDbfsRequest()
      .setPath(path));
  }
   ...
   * call throws an exception with `INVALID_PARAMETER_VALUE`. If the read length
