...
public class GetRuleSetRequest {
   ...
   * read -&gt; modify -&gt; write pattern to perform rule set updates in order to avoid race
   * conditions that is get an etag from a GET rule set request, and pass it with the PUT update
   * request to identify the rule set version you are updating.
   *
   * <p>Examples | Summary :--- | :--- {@code etag=} | An empty etag can only be used in GET to
   * indicate no freshness requirements. {@code
   * etag=RENUAAABhSweA4NvVmmUYdiU717H3Tgy0UJdor3gE4a+mq/oj9NjAf8ZsQ==} | An etag encoded a specific
   * version of the rule set to get or to be updated.
   */
  @JsonIgnore
  @QueryParam("etag")
  private String etag;
   ...
   *
   * <p>Examples | Summary :--- | :--- {@code name=accounts/<ACCOUNT_ID>/ruleSets/default} | A name
   * for a rule set on the account. {@code
   * name=accounts/<ACCOUNT_ID>/groups/<GROUP_ID>/ruleSets/default} | A name for a rule set on the
   * group. {@code
   * name=accounts/<ACCOUNT_ID>/servicePrincipals/<SERVICE_PRINCIPAL_APPLICATION_ID>/ruleSets/default}
   * | A name for a rule set on the service principal. {@code
