 ...
 * releases. * `SUCCESS`: The run was completed successfully. * `SUCCESS_WITH_FAILURES`: The run was
 * completed successfully but some child runs failed. * `USER_CANCELED`: The run was successfully
 * canceled during execution by a user. * `CANCELED`: The run was canceled during execution by the
 * Databricks platform; for example, if the maximum run duration was exceeded. * `SKIPPED`: Run was
 * never executed, for example, if the upstream task run failed, the dependency type condition was
 * not met, or there were no material tasks to execute. * `INTERNAL_ERROR`: The run encountered an
 * unexpected error. Refer to the state message for further details. * `DRIVER_ERROR`: The run
 * encountered an error while communicating with the Spark Driver. * `CLUSTER_ERROR`: The run failed
 ...
 * with the third party service. * `INVALID_CLUSTER_REQUEST`: The run failed because it issued an
 * invalid request to start the cluster. * `WORKSPACE_RUN_LIMIT_EXCEEDED`: The workspace has reached
 * the quota for the maximum number of concurrent active runs. Consider scheduling the runs over a
 * larger time frame. * `FEATURE_DISABLED`: The run failed because it tried to access a feature
 * unavailable for the workspace. * `CLUSTER_REQUEST_LIMIT_EXCEEDED`: The number of cluster
 * creation, start, and upsize requests have exceeded the allotted rate limit. Consider spreading
 * the run execution over a larger time frame. * `STORAGE_ACCESS_ERROR`: The run failed due to an
 * error when accessing the customer blob storage. Refer to the state message for further details. *
