...
public class LogDeliveryStatus {
   ...
   *
   * <ul>
   *   <li>{@code CREATED}: There were no log delivery attempts since the config was created.
   *   <li>{@code SUCCEEDED}: The latest attempt of log delivery has succeeded completely.
   *   <li>{@code USER_FAILURE}: The latest attempt of log delivery failed because of
   *       misconfiguration of customer provided permissions on role or storage.
   *   <li>{@code SYSTEM_FAILURE}: The latest attempt of log delivery failed because of an
   *       Databricks internal error. Contact support if it doesn't go away soon.
   *   <li>{@code NOT_FOUND}: The log delivery status as the configuration has been disabled since
   *       the release of this feature or there are no workspaces in the account.
   * </ul>
   */
  @JsonProperty("status")
