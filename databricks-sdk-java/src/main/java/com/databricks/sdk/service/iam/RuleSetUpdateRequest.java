...
public class RuleSetUpdateRequest {
   ...
   * suggested that systems make use of the etag in the read -&gt; modify -&gt; write pattern to
   * perform rule set updates in order to avoid race conditions that is get an etag from a GET rule
   * set request, and pass it with the PUT update request to identify the rule set version you are
   * updating.
   */
  @JsonProperty("etag")
  private String etag;
