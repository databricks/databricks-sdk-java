...
public class DataPlaneTokenSourceTest {
    ...
    private static Stream<Arguments> provideDataPlaneTokenScenarios() throws Exception {
        ...
        Token cpToken = new Token(TEST_CP_TOKEN, TEST_TOKEN_TYPE, null, LocalDateTime.now().plusSeconds(600));
        ...
        when(mockSuccessClient1.execute(any())).thenReturn(
                new Response(successJson1, 200, "OK", new URL(TEST_ENDPOINT_1))
        );
        ...
        when(mockSuccessClient2.execute(any())).thenReturn(
                new Response(successJson2, 200, "OK", new URL(TEST_ENDPOINT_2))
        );
        ...
        when(mockErrorClient.execute(any())).thenReturn(
                new Response(errorJson, 400, "Bad Request", new URL(TEST_ENDPOINT_1))
        );
        ...
        return Stream.of(
            Arguments.of(
                ...
                mockSuccessClient1,
                mockCpTokenSource,
                new Token("dp-access-token1", TEST_TOKEN_TYPE, TEST_REFRESH_TOKEN, LocalDateTime.now().plusSeconds(TEST_EXPIRES_IN)),
                null // No exception
            ),
            Arguments.of(
                ...
                mockSuccessClient2,
                mockCpTokenSource,
                new Token("dp-access-token2", TEST_TOKEN_TYPE, TEST_REFRESH_TOKEN, LocalDateTime.now().plusSeconds(TEST_EXPIRES_IN)),
                null
            ),
            Arguments.of(
