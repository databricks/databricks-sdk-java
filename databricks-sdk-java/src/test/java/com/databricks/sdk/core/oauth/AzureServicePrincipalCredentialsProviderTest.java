...
class AzureServicePrincipalCredentialsProviderTest {
    ...
    private static RefreshableTokenSource mockTokenSource() {
        ...
        Mockito.when(tokenSource.getToken()).thenReturn(new Token(TOKEN, TOKEN_TYPE, LocalDateTime.now().plus(1, IsoFields.WEEK_BASED_YEARS)));
        return tokenSource;
    }
    private static AzureServicePrincipalCredentialsProvider getAzureServicePrincipalCredentialsProvider(RefreshableTokenSource tokenSource) {
        AzureServicePrincipalCredentialsProvider provider = Mockito.spy(new AzureServicePrincipalCredentialsProvider());
    ...
    void testWorkSpaceIDUsage() {
        AzureServicePrincipalCredentialsProvider provider = getAzureServicePrincipalCredentialsProvider(mockTokenSource());
        ...
        Mockito.verify(provider, times(1)).tokenSourceFor(any(), eq(PUBLIC_MANAGEMENT_ENDPOINT), eq(SUBSCRIPTION));
    ...
    void testFallbackWhenTailsToGetTokenForSubscription() {
        ...
        AzureServicePrincipalCredentialsProvider provider = getAzureServicePrincipalCredentialsProvider(tokenSource);
    ...
    void testGetTokenWithoutWorkspaceResourceID() {
        ...
        Mockito.verify(provider, never()).tokenSourceFor(any(), eq(PUBLIC_MANAGEMENT_ENDPOINT), eq(SUBSCRIPTION));
        Mockito.verify(provider, times(1)).tokenSourceFor(any(), eq(ARM_DATABRICKS_RESOURCE_ID));
        Mockito.verify(provider, times(1)).tokenSourceFor(any(), eq(PUBLIC_MANAGEMENT_ENDPOINT));
    }
